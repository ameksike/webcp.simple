<!-- < GRID -->
<link rel="stylesheet" type="text/css" href="lib/datatables/DataTables-1.10.16/css/jquery.dataTables.min.css">
<link rel="stylesheet" type="text/css" href="lib/grid/phoneuser.css">
<!-- >

GRID -->

<table id="phoneuser" class="display" cellspacing="1" width="100%">
			<thead>
			<tr>
				<th width="10%">Foto</th>
				<th>Alia</th>
				<th>Nombre</th>
				<th>Apellidos</th>
				<th>Denominacion</th>
				<th>Numero</th>
				<th>Extension</th>
				<th>Notas</th>
				<th>Clasificacion</th>
				<th>Empresa</th>
				<th>Ciudad</th>
				<th>Tipo</th>
			</tr>
			</thead>

			<tfoot>
			<tr>
				<th>Foto</th>
				<th>Alia</th>
				<th>Nombre</th>
				<th>Apellidos</th>
				<th>Denominacion</th>
				<th>Numero</th>
				<th>Extension</th>
				<th>Notas</th>
				<th>Clasificacion</th>
				<th>Empresa</th>
				<th>Ciudad</th>
				<th>Tipo</th>
			</tr>
			</tfoot>

			<tbody></tbody>
		</table>

<script type="application/javascript" src="lib/datatables/jQuery-3.2.1/jquery-3.2.1.min.js" ></script>
<!-- < GRID -->
<script type="application/javascript" src="lib/datatables/DataTables-1.10.16/js/jquery.dataTables.min.js" ></script>
<script type="application/javascript" src="lib/grid/phoneuser.js" > </script>
<!-- > GRID -->